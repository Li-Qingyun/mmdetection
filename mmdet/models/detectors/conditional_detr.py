...
from ..layers import (ConditionalDetrTransformerDecoder, DetrTransformerEncoder,
                      SinePositionalEncoding)
from .detection_transformer import TransformerDetector


@MODELS.register_module()
class ConditionalDETR(TransformerDetector):
    r"""Implementation of `DETR: End-to-End Object Detection with
    Transformers <https://arxiv.org/pdf/2005.12872>`_"""

    def __init__(self, *args, **kwargs) -> None:
        super(ConditionalDETR, self).__init__(*args, **kwargs)
    ...
    def forward_transformer(
        ...
        if return_memory:
            ...
            return InstanceData(outs_trans=tuple([out_dec]), memory=tuple([memory]), reference_points=tuple(reference_points))
        return InstanceData(outs_trans=tuple([out_dec]), reference_points=tuple(reference_points))
